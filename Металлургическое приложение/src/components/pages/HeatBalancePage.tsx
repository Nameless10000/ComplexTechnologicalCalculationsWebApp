...
export function HeatBalancePage() {
  const { history, addToHistory, removeFromHistory, clearHistory } = useCalculationHistory('heat-balance');
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [calculationResults, setCalculationResults] = useState<any>(null);
  
  const handleSaveToHistory = (note: string) => {
    if (calculationResults) {
      addToHistory(note, 'Результаты теплового баланса');
    }
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Исходные данные для расчета теплообмена
                </CardDescription>
              </CardHeader>
              <CardContent>
      ...
      <SaveCalculationDialog
        ...
        resultsPreview="Результаты теплового баланса"
      />
    </div>
  );
}