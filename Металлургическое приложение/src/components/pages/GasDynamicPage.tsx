...
import { Plus, Trash2, Wind, Activity, Flame, Gauge, Package, Droplets, ArrowUpDown, TrendingUp, Factory, AlertCircle, Loader2 } from 'lucide-react';
...
export function GasDynamicPage() {
  ...
  const [calculationError, setError] = useState('');
  ...
  useEffect(() => {
    ...
    gasDynamicService.getPreset()
        .then(response => {
          setPresetInputs(response.data);
        })
        .catch(error => {
          setError(error.message || 'Произошла ошибка при расчете.');
        })
    
  }, [])

  // Уставнока пресета
  const setPresetInputs = (preset: any) => {
    setAglomContents(preset.aglomKontents); 
    setKoksContents(preset.koksContents); 
    setOkatContents(preset.okatContents); 
    setComposition(preset.composition); 
    setFuelAndBlast(preset.fuelAndBlast); 
    setGeometry(preset.geometry); 
    setThermalAndPressure(preset.thermalAndPressure); 
